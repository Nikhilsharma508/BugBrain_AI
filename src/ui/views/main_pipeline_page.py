...
def render():
    ...
    def render_status_item(slot, stage_name, description, is_complete=False, is_active=False):
        icon = "✅" if is_complete else ("🔄" if is_active else "⏳")
        border_color = "#64d9ff" if is_complete else ("#ffc107" if is_active else "rgba(100, 150, 220, 0.3)")
        bg_color = "rgba(50, 100, 80, 0.3)" if is_complete else ("rgba(150, 100, 20, 0.2)" if is_active else "rgba(10, 20, 35, 0.5)")
        text_color = "#64d9ff" if is_complete else ("#ffc107" if is_active else "#b0c4de")
        slot.markdown(
             f"""
    ...
    if not submitted:
        if has_result:
            ...
            render_status_item(status_slots["preprocess"], "Preprocessing", "[Filter] Stripped emotion, memory dumps.\n[Logger] Logged regex rules applied.", True)
            render_status_item(status_slots["extract"], "Extraction", "[LangChain] Extracting structured JSON.\nSummary created.", True)
            render_status_item(status_slots["duplicate_detection"], "Duplicate Detection", f"[FAISS] Vector similarity search initiated.\nThreshold: {int(EMBEDDING_SIMILARITY_THRESHOLD * 100)}%. Matched: {len(similar_reports)}", True)
            render_status_item(status_slots["triage"], "Triage", "[Policy] Severity rules applied.\nRouting suggested.", True)
        else:
            render_status_item(status_slots["preprocess"], "Preprocessing", "Waiting for input...", False)
            render_status_item(status_slots["extract"], "Extraction", "Waiting for input...", False)
            render_status_item(status_slots["duplicate_detection"], "Duplicate Detection", "Waiting for input...", False)
            render_status_item(status_slots["triage"], "Triage", "Waiting for input...", False)
    ...
    if submitted:
        ...
        has_result = False
        render_status_item(status_slots["preprocess"], "Preprocessing", "Processing incoming text...", False, True)
        render_status_item(status_slots["extract"], "Extraction", "Waiting for input...", False, False)
        render_status_item(status_slots["duplicate_detection"], "Duplicate Detection", "Waiting for input...", False, False)
        render_status_item(status_slots["triage"], "Triage", "Waiting for input...", False, False)
        ...
        try:
            ...
            for event in run_pipeline(bug_trace=bug_trace, user_review=user_review):
                ...
                if node == "preprocess":
                    render_status_item(status_slots["preprocess"], "Preprocessing", "[Filter] Stripped emotion, memory dumps.\n[Logger] Logged regex rules applied.", True)
                    render_status_item(status_slots["extract"], "Extraction", "Calling LLM for extraction...", False, True)
                    
                elif node == "extract":
                    render_status_item(status_slots["extract"], "Extraction", "[LangChain] Extracting structured JSON.\nSummary created.", True)
                    render_status_item(status_slots["duplicate_detection"], "Duplicate Detection", "Searching FAISS vector DB...", False, True)
                    
                elif node == "duplicate_detection":
                    state = event.get("current_state", {})
                    sim = state.get("similar_reports", [])
                    render_status_item(status_slots["duplicate_detection"], "Duplicate Detection", f"[FAISS] Vector similarity search finished.\nThreshold: {int(EMBEDDING_SIMILARITY_THRESHOLD * 100)}%. Matched: {len(sim)}", True)
                    render_status_item(status_slots["triage"], "Triage", "Applying severity policies...", False, True)
                    ...
                    with st.spinner("Committing to standard dataset & vector store..."):
                        try:
                            ...
                            new_row = pd.DataFrame([{
                                ...
                                "User Review": str(user_review) if user_review else "No user review provided"
                            }])
