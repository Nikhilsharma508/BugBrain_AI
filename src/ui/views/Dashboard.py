...
def render():
    ...
    if df.empty:
        st.markdown("<div class='metric-card' style='text-align: center;'><h3>No Data Found</h3><p>Run the triage pipeline to generate analytics.</p></div>", unsafe_allow_html=True)
        return
    ...
    with m1:
        st.markdown(f"""<div class='metric-card'><div class='metric-val'>{len(df)}</div><div class='metric-lab'>Reports handled</div></div>""", unsafe_allow_html=True)

    with m2:
        top_sev = df["Severity"].mode()[0] if not df["Severity"].empty else "N/A"
        st.markdown(f"""<div class='metric-card'><div class='metric-val' style='color: #ff9999;'>{top_sev}</div><div class='metric-lab'>Main Impact</div></div>""", unsafe_allow_html=True)

    with m3:
        top_owner = df["Owner"].mode()[0] if not df["Owner"].empty else "N/A"
        st.markdown(f"""<div class='metric-card'><div class='metric-val' style='color: #99ff99;'>{top_owner.split('-')[0]}</div><div class='metric-lab'>Lead Assignee</div></div>""", unsafe_allow_html=True)

    with m4:
        avg_char = int(df["Summary"].str.len().mean()) if not df["Summary"].empty else 0
        st.markdown(f"""<div class='metric-card'><div class='metric-val' style='color: #64d9ff;'>{avg_char}</div><div class='metric-lab'>Avg Length</div></div>""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        chart1 = alt.Chart(sev_counts).mark_bar(size=55).encode(
    ...
    with c2:
        ...
        chart2 = alt.Chart(team_counts).mark_bar(size=55).encode(
            ...
            fill="transparent"   # removes black background
        ).configure(
            background="transparent"  # makes full chart transparent
        )
    ...
    raw_df = df[["Id", "Severity", "Owner", "Summary", "Timestamp"]] \
