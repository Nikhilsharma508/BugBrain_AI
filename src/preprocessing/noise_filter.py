...
NOISE_PATTERNS = [
    ...
    re.compile(r"^[0-9a-f]{8,16}-[0-9a-f]{8,16}\s+[\w-]{4}\s+[0-9a-f]+.*$", re.MULTILINE | re.IGNORECASE),
    ...
    re.compile(r"^.*[/\\](?:lib|bin|plugins|node_modules|site-packages)[/\\][\w.\-]+\.(?:so|dll|jar|pyc|exe|dylib)\s*$", re.MULTILINE | re.IGNORECASE),

    # 4. Runtime Memory/Resource Summaries
    re.compile(r"^\s*(?:Heap|GC|Thread|Memory|Object|Handle|PSYoungGen|ParOldGen|total|used|eden|from|to)\s+(?:summary|total|count|info|stats|:)\s*.*$", re.MULTILINE | re.IGNORECASE),
    
    # 5. Low-level Thread/Register Dumps
    re.compile(r"^.*(?:prio=\d+|tid=0x|nid=0x|rax=|rbx=|rcx=|eip=|esp=).*$", re.MULTILINE | re.IGNORECASE),
